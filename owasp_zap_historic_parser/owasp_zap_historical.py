...
def process_zap_file(opts):
    ...
    root_ozhdb = connect_to_mysql_db(
        opts.ozh_host,
        opts.ozh_port,
        opts.ozh_username,
        opts.ozh_password,
    ...
    final_message = process_zap_results(
        my_ozhdb,
        root_ozhdb,
        opts.this_env,
        opts.scantype,
...
def get_locators(base_loc, loc_index):
    ...
    if base_loc in [HIGH_LOCATOR, MED_LOCATOR, LOW_LOCATOR, INFO_LOCATOR]:
        ...
        url_count_loc = (
            "(" + base_loc + ")[" + str(loc_index + 1) + "]/../..//td[.='URL']"
        )
    elif base_loc in [
        NEW_HIGH_LOCATOR,
        NEW_MED_LOCATOR,
...
def process_zap_results(
    con, ocon, this_env, scantype, zapresults, projectname, url_link, version
):
    ...
    cursor_obj.execute(
        "INSERT INTO TB_EXECUTION (Execution_Id, Execution_Date) "
        "VALUES (0, '%s')" % utc
    )
    con.commit()
    cursor_obj.execute(
        "SELECT Execution_Id, Execution_Date from TB_EXECUTION ORDER BY "
    ...
    for result in zapresults:
        ...
        sql = (
            "INSERT INTO TB_ALERTS (Alert_Id, Execution_Id, Alert_Level, Alert_Type,"
            " URLS_Affected)"
            "VALUES (%s, %s, %s, %s, %s);"
        )
    ...
    cursor_obj.execute(
        "SELECT COUNT(*) FROM TB_ALERTS WHERE Execution_Id = '%s' AND"
    ...
    cursor_obj.execute(
        "UPDATE TB_EXECUTION SET Environment = '%s', Scan_Type = '%s',"
    ...
    title = (
        "<h1>OWASP ZAP Report Comparison for "
        + this_env
        + " / "
        + scantype
        + " / "
        + version
        + "</h1><hr /><table style='border: 1px white; border-collapse: collapse;'>"
        + "<thead>"
        + "</thead><tbody><tr><td style='border: 1px;'><strong>This report date: "
        + "</strong></td>"
        + "<td style='border: 1px;'>"
        + last_date
        + "</td></tr><tr>"
        + "<td style='border: 1px;'>"
        + "<strong>This report link:</strong></td><td style"
        + "='border: 1px;'><a href='"
        + url_link.replace(" ", "%20")
        + "'>This ZAP Report"
        + "</a></td></tr>"
    ...
    alert_breakdown = ""
    cursor_obj.execute(
        "SELECT COUNT(*) FROM TB_EXECUTION WHERE Environment = '%s'"
        " AND Scan_Type ='%s' ;" % (this_env, scantype)
    )
    compare_rows = cursor_obj.fetchone()
    if compare_rows[0] < 2:
        title += (
            "</tbody></table><p>Not enough rows to compare results for "
            + this_env
            + " and "
            + scantype
            + ".</p><hr />"
        ...
        cursor_obj.execute(
            "SELECT Execution_Id, Execution_Date, URL_Link FROM TB_EXECUTION "
        ...
        cursor_obj.execute(
            "SELECT Alert_level, Alert_Type, URLS_Affected FROM TB_ALERTS WHERE "
        ...
        cursor_obj.execute(
            "SELECT Version FROM TB_EXECUTION WHERE Execution_Id = '%s'"
            % compare_row[0]
        )
        ...
        title += (
            "<tr><td style='border: 1px;'><strong>Comparison Report Version:</strong></td>"
            + "<td style='border: 1px;'>"
            + str(last_version[0])
            + "</td></tr><tr>"
        ...
        overall = (
            "<h2>Overall Alerts</h2><table style='float: left; text-align: center; "
            ...
            + str(total_alerts)
            + "</strong></td><td style='border: 1px solid black; "
            + "'><strong>"
            + str(high_alerts[0])
            + "</strong></td><td style='"
            ...
            + str(false_alerts[0])
            + "</strong></td></tr></tbody></table><br><br><br><hr />"
        )
        current_dict = convert_alert_to_dictionary(current_alerts)
        last_dict = convert_alert_to_dictionary(last_alerts)
        alert_breakdown = compare_zap_results(
            current_dict, last_dict, last_date, compare_date
        )
...
def compare_zap_results(set1, set2, date1, date2):
    ...
    alerts_table = (
        "<h2>Alert Breakdown</h2><table style='float: left; text-align: center; "
        ...
        + date1
        + "</strong></td><td style='border: 1px solid;'><strong>URLs "
        + "Affected<br />"
        + date2
    ...
    for key in set1:
        if key in set2:
            if set2[key]["Alert Level"] == "High":
                high_alerts += get_alert_table_row(
                    "red",
                    "#FFF",
                    "High",
                    set1[key]["Alert Type"],
                ...
                med_alerts += get_alert_table_row(
                    "orange",
                    "#FFF",
                    "Medium",
                    set1[key]["Alert Type"],
                ...
                low_alerts += get_alert_table_row(
                    "yellow",
                    "#000",
                    "Low",
                    set1[key]["Alert Type"],
                ...
                info_alerts += get_alert_table_row(
                    "blue",
                    "#FFF",
                    "Informational",
                    set1[key]["Alert Type"],
                ...
                false_alerts += get_alert_table_row(
                    "green",
                    "#FFF",
                    "False Positive",
                    set1[key]["Alert Type"],
    ...
    for key in set2:
        if key not in set1:
            resolved_alerts += get_alert_table_row(
                "lightgreen",
                "#000",
...
def connect_to_mysql_db(host, port, user, pwd, dbname):
    ...
    try:
        return mysql.connector.connect(
            host=host, port=port, user=user, passwd=pwd, database=dbname
        )
