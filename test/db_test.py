...
from owasp_zap_historic_parser.owasp_zap_historical import (
    connect_to_mysql_db,
    process_zap_file,
    process_zap_results,
    html_parser,
...
class TestDBFunctions(unittest.TestCase):
    ...
    @patch("mysql.connector.connect")
    @patch(
        "owasp_zap_historic_parser.owasp_zap_historical.process_zap_results",
    ...
    def test_process_zap_results_compare_same(self, mock_conn):
        ...
        mock_cursor.fetchone.side_effect = [
            (1, datetime.datetime(2018, 1, 6, 5, 5, 5)),
            (2,),
            (2,),
            (2,),
            (2,),
        ...
        mock_cursor.fetchall.side_effect = [
            [
                (1, datetime.datetime(2018, 3, 6, 5, 5, 5), "http://www.google.com"),
                (1, datetime.datetime(2018, 4, 6, 5, 5, 5), "http://www.google.com"),
            ],
            [("High", "Test Alert A", 3), ("Medium", "Test Alert B", 6)],
            [("High", "Test Alert A", 3), ("Medium", "Test Alert B", 6)],
        ]
        mock_cursor.description = (("name",), ("title",))
        zap_results = html_parser(file_path)
    ...
    def test_process_zap_results_compare_increase(self, mock_conn):
        ...
        mock_cursor.fetchall.side_effect = [
            ...
            [("Low", "Test Alert A", 3), ("Informational", "Test Alert B", 6)],
            [("Low", "Test Alert A", 2), ("Informational", "Test Alert B", 5)],
        ]
        mock_cursor.description = (("name",), ("title",))
        zap_results = html_parser(file_path)
        args = (
    ...
    def test_process_zap_results_compare_decrease(self, mock_conn):
        ...
        mock_cursor.fetchall.side_effect = [
            ...
            [("High", "Test Alert A", 1), ("False Positive", "Test Alert B", 4)],
            [("High", "Test Alert A", 2), ("False Positive", "Test Alert B", 5)],
        ]
        mock_cursor.description = (("name",), ("title",))
        zap_results = html_parser(file_path)
        args = (
