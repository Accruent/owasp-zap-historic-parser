...
from owasp_zap_historic_parser.owasp_zap_historical import connect_to_mysql_db, process_zap_file, process_zap_results, \
    html_parser
...
class TestDBFunctions(unittest.TestCase):
    ...
    @patch('mysql.connector.connect')
    def test_process_zap_results_compare(self, mock_conn):
        """Tests the process zap file function"""
        file_path = ROOT_PATH + "/" + "test_files/testReport.html"
        mock_cursor = mock.Mock()
        mock_conn.cursor.return_value = mock_cursor
        mock_cursor.fetchone.side_effect = [(1, datetime.datetime(2018, 5, 6, 5, 5, 5)),
                                            (2, ), (2, ), (2, ), (2, ), (2, ),
                                            (2, datetime.datetime(2018, 5, 6, 5, 5, 5)),
                                            ('test', )]
        mock_cursor.fetchall.return_value = [(1, datetime.datetime(2018, 5, 6, 5, 5, 5)),
                                             (1, datetime.datetime(2018, 5, 6, 5, 5, 5),
